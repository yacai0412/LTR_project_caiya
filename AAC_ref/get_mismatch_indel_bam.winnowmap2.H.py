...
def main():
    ...
    inbam = sys.argv[2]
    fastq_fasta = sys.argv[3] # fastq.gz / fq.gz / fastq / fq / fasta.gz / fa.gz / fasta / fa
    snv_out = inbam.split("/")[-1] + ".snv"
    indel_out = inbam.split("/")[-1] + ".indel"

    ref_dic = read_fasta(ref_fasta)
    if fastq_fasta.endswith('fastq.gz') or fastq_fasta.endswith('fq.gz') or fastq_fasta.endswith('fastq') or fastq_fasta.endswith('fq'):
        fastq_fasta_len_dic = read_fastq_get_rlen(fastq_fasta)
    elif fastq_fasta.endswith('fastq.gz') or fastq_fasta.endswith('fq.gz') or fastq_fasta.endswith('fastq') or fastq_fasta.endswith('fq'):
        fastq_fasta_len_dic = read_fasta_get_rlen(fastq_fasta)

    read_bam_snv_indel(inbam, fastq_fasta_len_dic, ref_dic, snv_out, indel_out)
...
def read_fasta_get_rlen(fasta):
    ...
    with open_func(fasta, mode) as inf:
        for line in inf:
            line = line.rstrip("\n")
            if line.startswith(">"):
                name = line.lstrip(">")
                ...
                ref_dic[name] = ref_dic[name] + len(line)
    return ref_dic
...
def read_bam_snv_indel(inbam, fastq_len_dic, ref_dic, snv_out, indel_out):
    with pysam.AlignmentFile(inbam, "rb") as samfile, open(snv_out, "w") as snv_final, open(indel_out, "w") as indel_final:
        for read in samfile:
