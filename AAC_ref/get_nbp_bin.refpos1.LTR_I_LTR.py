...
def main():
    ...
    out = in_refpos1.split("/")[-1] + "." + str(bin_size) + "bp.bin.refpos1"
    get_binsize_refpos1_LTR_I_LTR(in_refpos1, bin_size, out)
...
def get_binsize_refpos1_LTR_I_LTR(in_refpos1, bin_size, out):
    df = pd.read_csv(in_refpos1, sep='\t', header=None)
    df.columns = ['Label', 'Position', 'LTR', 'mutation_count', 'depth', 'mutation_rate']
    binned_results = []

    for (label, ltr), group in df.groupby(['Label', 'LTR']):
        binned_group = bin_data(group, bin_size)
        binned_results.append(binned_group)
...
def bin_data(df, bin_size):
    ...
    for start in range(min_position, max_position, bin_size):
        ...
        if not bin_df.empty:
            ...
            binned_data.append([label, median_position, ltr, sum_col5, sum_col6, mean_mutation_rate])

    # Convert the binned data back into a DataFrame
    binned_df = pd.DataFrame(binned_data, columns=['Label', 'Position', 'LTR', 'sum_mutation_count', 'sum_depth', 'mean_mutation_rate'])
