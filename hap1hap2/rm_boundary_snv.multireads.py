...
def main():
    ...
    rm_length = int(sys.argv[3]) # 25?
    in_snv_list = sys.argv[4] 

    readlength_dic = get_readlength_fastq(in_fastq)
    rm_boundary_snv(in_snv_list, readlength_dic, rm_length)
...
def rm_boundary_snv(in_snv_list, readlength_dic, rm_length):
    with open(in_snv_list, "r") as snvs:
        for snv in snvs:
            snv = snv.rstrip("\n")
            out = ".".join(snv.split(".")[:-1]) + ".rm_" + str(rm_length) + "bp.out"
            final = open(out, "w")
                ...
                for line in inf:
                    ...
                    if len(ll) == 7:
                        ...
                        for i in range(len(support_reads)):
                            ...
                            readname = read.split(":")[0]
                            readpos = int(read.split(":")[1])
                            if readname in readlength_dic:
                                readlength = readlength_dic[readname]
                                relative_length = min(readpos, abs(readlength - readpos))
