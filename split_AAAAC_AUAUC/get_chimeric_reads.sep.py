...
def main():
    ...
    infastqgz = sys.argv[3] # /rd/caiya/LTR/duplex/AA.deepconsensus.cx3.fwd.fastq.gz / /rd/caiya/LTR/duplex/AAC_ref/AA.deepconsensus.cx3.q30.fastq.gz
    output_tag = sys.argv[4] # tegene

    fastq_out = ".".join(infastqgz.split("/")[-1].split(".")[:-2]) + "." + output_tag + ".chimeric.split.fastq"
    tab_out = ".".join(intab.split("/")[-1].split(".")[:-1]) + ".rm_no_split.q30.tab"
    bed_out = ".".join(intab.split("/")[-1].split(".")[:-1]) + ".rm_no_split.q30.bed"

    seq_dict = parse_fastq_gz(infastqgz)
    no_split_readname_dic = get_nosplit_readnames_dic(no_split_readname)
    fwd_rev_paired_dic = read_intab_filter_nosplit_reads(intab, no_split_readname_dic)
...
def read_intab_filter_nosplit_reads(intab, no_split_readname_dic):
    fwd_rev_paired_dic = {}
