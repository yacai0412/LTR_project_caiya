...
def main():
    ...
    indel_pos_length_cutoff = int(sys.argv[3])

    # out = read_specific_snv.split("/")[-1] + ".inside_indel.del0." + str(indel_pos_length_cutoff) +  "bp.relativepos"
    out = sys.argv[4]

    indel_pos_dic = read_inside_indel(inside_indel_pos, indel_pos_length_cutoff)
...
def get_indel_start_end_pos(relative_pos_column):
    ...
    return length, str(indel_start) + "\t" + str(indel_end)

def read_inside_indel(inside_indel_pos, indel_pos_length_cutoff):
    indel_pos_dic = {}
    with open(inside_indel_pos, "r") as inf:
        for line in inf:
            ll = line.rstrip("\n").split("\t")
            rname = ll[0]
            indel_length, indel_start_end = get_indel_start_end_pos(ll[2])
            if indel_length >= indel_pos_length_cutoff:
...
def get_nearest_indel(relative_pos, indel_dic):
    ...
    for indel_start_end in indel_dic:
        ...
        indel_type0 = indel_dic[indel_start_end].split("_")[1]
        if indel_type0 == "insertion":
        ...
        elif indel_type0 == "deletion":
            if (relative_pos > indel_start and relative_pos > indel_end) or relative_pos == indel_end:
                direction = "right"
                distance = relative_pos - indel_end + 1
            elif (relative_pos < indel_start and relative_pos < indel_end) or relative_pos == indel_start:
                direction = "left"
                distance = indel_start - relative_pos + 1
